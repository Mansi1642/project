...
function showSales($start_date, $end_date) {
  ...
  if($con) {
    ...
    $total = 0;
    if($start_date == "" || $end_date == "")
      $query = "SELECT * FROM invoices INNER JOIN users ON invoices.CUSTOMER_ID = users.ID";
    else
      $query = "SELECT * FROM invoices INNER JOIN users ON invoices.CUSTOMER_ID = users.ID WHERE INVOICE_DATE BETWEEN '$start_date' AND '$end_date'";
	// die($query);
    $result = mysqli_query($con, $query);
	// print_r($result);
	// die("xcv");
